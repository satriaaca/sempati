...
# 2. KOP SURAT (Menggunakan Tabel tanpa border untuk presisi Logo & Teks)
    ...
    p_nomor.add_run(f"NOMOR : {form_data['nomor_surat']}")
    ...
    p_naskah.add_run(f"Nomor     :  {form_data['nomor_laporan']}\n")
    p_naskah.add_run(f"Tanggal   :  {form_data['tanggal_laporan']}\n")
    p_naskah.add_run(f"Perihal     :  {form_data['perihal_laporan']}")
    ...
    p_pejabat.add_run(f"{form_data['ttd_jabatan']}\n\n")
    ...
    run_nama = p_pejabat.add_run(f"{form_data['ttd_nama']}\n")
    run_nama.bold = True
    run_nama.underline = True
    
    p_pejabat.add_run(f"{form_data['ttd_nip']}")
...
# --- Blok eksekusi utama skrip ---
if __name__ == "__main__":
    # Payload data dummy untuk pengetesan langsung
    dummy_payload = {
        "tanggal_surat": "1 Juli 2026",
        "nomor_surat": "R-250/N.1.17/Dip.4/07/2026",
        "nama_laporan": "LAPORAN INFORMASI KHUSUS",
        "nomor_laporan": "R-LIK-1094/N.1.17/Dip.4/07/2026",
        "tanggal_laporan": "1 Juli 2026",
        "perihal_laporan": "Pengamanan Sidang Tindak Pidana Perdagangan Impor Barang Dalam Bentuk Pakaian Bekas (thrifting) 1 Juli 2026.",
        "jumlah": "1 (satu)\neks.",
        "keterangan": "Dengan hormat kami laporkan, selanjutnya mohon petunjuk pimpinan.",
        "ttd_jabatan": "Kepala Kejaksaan Negeri Tabanan",
        "ttd_nama": "Medie, S.H, M.H.",
        "ttd_nip": "Jaksa Madya NIP. 19710524 199003 1 001",
        "tembusan": [
    ...
    # Menjalankan fungsi generator dokumen
    create_surat_pengantar(dummy_payload)